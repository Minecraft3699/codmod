...
public class BRGenUtil {
    ...
    public static void fillAreaRandom(WorldGenRegion worldGenRegion, ChunkAccess chunk, int startX, int startY, int startZ, int endX, int endY, int endZ, BlockState state, float chance) {
    ...
    public static void generateBasicWalls(ChunkAccess chunk, int floorLevel, int ceilingLevel, BlockState state) {
        ...
        if (random.nextBoolean()) {
            fillArea(chunk, startX, floorLevel + 1, startZ, startX + length, ceilingLevel - 1, startZ + wallThickness, state);
        } else {
            fillArea(chunk, startX, floorLevel + 1, startZ, startX + wallThickness, ceilingLevel - 1, startZ + length, state);
        }
    ...
    public static void generateBasicWallsWithExtension(ChunkAccess chunk, int floorLevel, int ceilingLevel, BlockState wallState, int extensionOffset, BlockState extensionState) {
        ...
        if (random.nextBoolean()) {
            fillArea(chunk, startX, floorLevel + 1, startZ, startX + length, ceilingLevel - 1, startZ + wallThickness, wallState);
            fillArea(chunk, startX, ceilingLevel + 1, startZ, startX + length, ceilingLevel + extensionOffset, startZ + wallThickness, extensionState);
        } else {
